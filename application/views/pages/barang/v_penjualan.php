...
<div class="row">
	<div class="col">
		<div class="">
			<a href="javascript:;" id="btn-show-add-form" class="btn btn-sm btn-success"><i class="icons icon-plus"></i>&nbsp;Tambah Penjualan</a>
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content ">
			<div class="modal-header bg-dark text-light">
				<h5 id="modal-label"><i class="flaticon2-avatar icon-2x text-grey-light"></i> Form Penjualan</h5>
			...
			<div class="modal-body">
					...
					<div>
						<div class="form-group">
							<label for="nama_penjual" class="form-control-label">Nama Penjual<span class="text-danger">*</span> :</label>
...
<div id="main-modal-marketing" class="modal fade" data-backdrop="static" data-keyboard="false" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" style="display: none;" aria-hidden="true">
	<div class="modal-dialog" role="document">
		<div class="modal-content ">
			<div class="modal-header bg-dark text-light">
				<h5 id="modal-label"><i class="flaticon2-avatar icon-2x text-grey-light"></i> Cetak Data Barang Keluar  </h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close"></button>
			</div>
...
<script>
	document.addEventListener('DOMContentLoaded', function() {
		table = $('#kt_table_1').DataTable({
			...
			ajax: {
				url: 'barang/pagination/penjualan',
				type: 'POST',
				data: function(e) {
					e.csrf_token = token
				}
		...
		$('#btn-show-add-form').click(function() {
			...
			$('#main-modal #modal-form').attr('action', 'barang/add/penjualan')
			$('#main-modal').modal()
		})
		...
		$(document).on('click', '.btn-edit', function() {
				...
				$('#main-modal #modal-form').attr('action', 'barang/update/penjualan');
				$('#main-modal').modal();

				// Kosongkan wrapper dulu
				$('#barang-wrapper').html('');

				fetch('barang/editPenjualan/' + id)
						.then(resp => resp.json())
						.then(data => {
								if (data && data.penjualan) {
										// Isi field utama
										$('#id_penjualan').val(data.penjualan.id);
										$('#nama_vendor').val(data.penjualan.nama_vendor);
										$('#alamat_vendor').val(data.penjualan.alamat_vendor);
										$('#tanggal').val(data.penjualan.tanggal);
										$('#nama_penjual').val(data.penjualan.nama_penjual);
		...
		$("#btn-export").click(function(){
                ...
                tglakhir = $("#tglakhir").val();
                 window.open("<?php echo base_url(); ?>barang/exportKeluar/search?tglawal="+encodeURIComponent(tglawal)+"&tglakhir="+encodeURIComponent(tglakhir),"_blank");
